...
func StartXML(g *config.Config) {
	if g.XmlFilename != "-" {
		g.OpenedXmlFile = OpenFile(g.XmlFilename)
	}
	portsLen := 1237
...
<verbose level="0"/>
<debugging level="0"/>
`, startstr, GetCommand(), GetCommand(), g.ScanStartTime.Unix(), startstr, portsLen, portsStr), g.XmlFilename, g.OpenedXmlFile)
}

func portToXML(port config.Port, result config.Output) string {
...
func ContinueXML(result config.Output, g *config.Config) {
	thisOutput := ""
...
`, result.Start.Unix(), result.End.Unix(), result.IP)
	...
	Write(thisOutput, g.XmlFilename, g.OpenedXmlFile)
}

func EndXML(g *config.Config) {
...
`, g.ScanEndTime.Unix(), timestr, elapsed, timestr, g.TotalHosts, g.AliveHosts, elapsed, g.AliveHosts, g.TotalHosts-g.AliveHosts, g.TotalHosts), g.XmlFilename, g.OpenedXmlFile)
	defer g.OpenedXmlFile.Close()
}
